...
import { CardContainer, CardBody, CardItem } from "./ui/3d-card"; // Ton chemin exact
  ...
  return (
    <>
      ...
      >
        <CardContainer containerClassName="py-8">
          <CardBody className="relative w-full max-w-[300px] h-[480px] bg-gradient-to-br from-gray-900/90 to-gray-950/90 backdrop-blur-xl rounded-3xl border border-emerald-500/20 shadow-2xl hover:shadow-emerald-500/30 hover:border-emerald-500/40 transition-all duration-500 overflow-hidden">
            ...
            {/* === BLOQUE LA PROPAGATION SUR LE CONTENU === */}
            <div
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            initial={{ opacity: 53, y: 30, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 30, scale: 0.95 }}
            transition={{ duration: 0.3, ease: "easeOut" }}
            className="fixed inset-x-0 bottom-8 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-4 p-4 rounded-xl shadow-2xl backdrop-blur-md border border-emerald-500/30 bg-gray-900/95 max-w-sm w-full mx-auto pointer-events-none"
          >
            ...
            <div className="flex flex-col">
              ...
              <p className="text-gray-400 text-xs mt-0.5 line-clamp-1">{product.name}</p>
            </div>
            <motion.div
              initial={{ width: "100%" }}
              animate={{ width: "0%" }}
              transition={{ duration: 2.5, ease: "linear" }}
              className="absolute bottom-0 left-0 right-0 h-1 bg-emerald-500 rounded-full origin-left"
            />
          </motion.div>
        )}
      </AnimatePresence>
